...
class NameServer(object):
	...
	def __init__(self, run_id, working_directory=None, host=None, port=0, self.nic_name=None):
		"""
	...
	def start(self):
		...
		if self.host is None:
			if self.nic_name is None:
				...
				self.host = hputil.nic_name_to_host(self.nic_name)

		uri, self.pyro_ns, _ = Pyro4.naming.startNS(host=self.host, port=self.port)

		self.host, self.port = ns.locationStr.split(':')
