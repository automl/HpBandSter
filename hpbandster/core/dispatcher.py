...
class Dispatcher(object):
  ...
  def __init__(self, new_result_callback, run_id='0',
    ...
    self.register_results_from_died_workers = register_results_from_died_workers
    self.id_to_job_dict = dict()
  ...
  def discover_workers(self):
    ...
    while True:
      ...
      for crashed_job in crashed_jobs:
        self.discover_cond.release()
        if self.register_results_from_died_workers:
          self.register_result(crashed_job, {'result': None, 'exception': 'Worker died unexpectedly.'})
        else:
          job = self.id_to_job_dict[crashed_job]
          self.submit_job(job.id, **job.kwargs)
        self.discover_cond.acquire()
  ...
  def submit_job(self, id, **kwargs):
    ...
    with self.runner_cond:
      ...
      if not self.register_results_from_died_workers:
        self.id_to_job_dict[job.id] = job
